...
func TestBoltDB_First(t *testing.T) {
	tmpfile := filepath.Join(os.TempDir(), "test.db")
	defer os.Remove(tmpfile)
	...
	s := BoltDB{DB: db, Channels: []string{"chan1", "chan2"}}
	_, err = s.First()
	assert.EqualError(t, err, "can't load entries for chan1: no bucket for chan1")
	...
	}
	{
		entry := feed.Entry{
			...
			VideoID:   "vid2",
			Title:     "title2",
			Published: time.Date(2022, time.March, 21, 17, 45, 23, 0, time.UTC),
			File:      "f2",
		}
		created, e := s.Save(entry)
		require.NoError(t, e)
	...
	res, err := s.First()
	require.NoError(t, err)
	assert.Equal(t, "vid1", res.VideoID)
}
